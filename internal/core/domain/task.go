...
func (t *Task) Validate() error {
	...
	if titleLen < 1 || titleLen > 100 {
		return fmt.Errorf("invalid 'Title' lenght: %d: %w", titleLen, core_errors.ErrInvalidArgument)
	}

	if t.Description != nil {
		descriptionLen := len([]rune(*t.Description))
		if descriptionLen < 1 || descriptionLen > 1000 {
			return fmt.Errorf("invalid 'Description' lenght: %d: %w", titleLen, core_errors.ErrInvalidArgument)
		}
	}
